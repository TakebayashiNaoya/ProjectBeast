...
namespace app
{
	namespace nature
	{
		...
		public:
			...
			/**
			 * @brief UV回転角度を直接設定する（ラジアン）
			 * @details 通常はUpdate()経由のStateMachine()が毎フレーム加算して渦を回転させて見せているが、
			 *          Update()を呼ばない外部運用（リプレイ再生など）では常に0のままになってしまうため、
			 *          外部から直接進められるようにする。
			 * @param uvRotation UV回転角度（ラジアン）
			 */
