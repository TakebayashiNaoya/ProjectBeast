#include "stdafx.h"
#include "Decal.h"
// ★削除: 深度バッファ・カメラ行列を使わなくなったのでCameraSystem.h/RenderingEngine.hは不要
...
namespace app {
	namespace effect {
			...
			int priority, const char* sharedTkmKey, const TerrainHeightInfo& terrainInfo)
		{
			...
			// ★変更: 箱ではなく板（Quad）を、地形法線に沿わせて配置する。
			...
			// ★デバッグ用: 回転もオフセットも一切無視して、確実に浮いた場所に出す
			//m_modelRender->SetTRS(pos + Vector3(0.0f, 50.0f, 0.0f), Quaternion::Identity, Vector3(size, 1.0f, size));
		...
		bool Decal::Update(float deltaTime) {
			...
			m_modelRender->SetExpandConstantBuffer2(&m_cb);
			m_modelRender->Update();
			// ★削除: 毎フレームのカメラ逆行列更新が不要に（そもそも計算していない）
			return true;
		}
