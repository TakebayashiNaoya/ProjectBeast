...
namespace app
{
	namespace util
	{
		...
		const Quaternion JsonConverter::InvalidQuaternion = Quaternion::Identity;
		...
		uint32_t JsonConverter::ToUInt32(
			const nlohmann::json& json,
			const char* key,
			uint32_t invalid
		)
		...
		std::string JsonConverter::ToString(
			const nlohmann::json& json,
			const char* key,
			std::string invalid
		)
		...
		Vector2 JsonConverter::ToVector2(
			const nlohmann::json& json,
			const char* key,
			Vector2 invalid
		)
		...
		Vector3 JsonConverter::ToVector3(
			const nlohmann::json& json,
			const char* key,
			bool isScale,
			Vector3 invalid
		...
		Quaternion JsonConverter::ToRotation(
			const nlohmann::json& json,
			const char* key,
			bool isDegree,
			Quaternion invalid
		)
		{
			...
			if (isDegree)
			{
				Vector3 rotDeg = ToVector3(json, key);
				if (rotDeg.IsEqual(InvalidVector3)) return invalid;
			...
			{
				if (!json[key].is_array() || json[key].size() != 4) { VALUE_DIFFER; return invalid; }

				const auto& array = json[key];
				for (size_t i = 0; i < 4; i++)
				{
					if (!array[i].is_number()) { VALUE_DIFFER; return invalid; }
				}
		...
		Vector4 JsonConverter::ToVector4(
			const nlohmann::json& json,
			const char* key,
			bool isConvert,
			Vector4 invalid
		)
		{
			...
			static constexpr float convertValue = 255.0f;
		...
		FloatRange JsonConverter::ToFloatRange(
			const nlohmann::json& json,
			const char* key
		)
		{
