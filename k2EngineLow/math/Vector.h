...
namespace nsK2EngineLow {
	...
	public:
		...
		bool IsEqual(const Vector3& v, const float value = 0.001f) const
		{
			return (fabsf(x - v.x) < value) && (fabsf(y - v.y) < value) && (fabsf(z - v.z) < value);
		}
	...
	public:
		...
		{
			return (fabsf(x - q.x) < value) && (fabsf(y - q.y) < value) && (fabsf(z - q.z) < value) && (fabsf(w - q.w) < value);
		}
