...
namespace nsK2EngineLow {
	...
	public:
		...
		/// <summary>
		/// エフェクトが存在（再生継続）しているか判定。
		/// GetShown()と異なりSetShown(false)の影響を受けない。
		/// </summary>
		/// <param name="handle">対象のエフェクトハンドル。</param>
		/// <returns>再生継続中ならtrue。</returns>
		bool IsExist(const int handle) const
		{
		...
		/// </summary>
		/// <param name="handle">対象のエフェクトハンドル。</param>
		/// <param name="shown">trueで描画する、falseで描画を抑制する。</param>
		void SetShown(const int handle, const bool shown)
		{
			m_manager->SetShown(handle, shown);
		}
		/// <summary>
		/// エフェクトのワールド行列を更新。
		/// </summary>
